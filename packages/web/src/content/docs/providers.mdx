...
OpenAI API keys and [ChatGPT Plus or Pro](https://chatgpt.com/pricing) subscriptions are configured as separate
providers.

#### Using API keys

Run the `/connect` command, select **OpenAI (API Key)**, and enter your API key.

```txt
/connect
```

#### Using ChatGPT Plus or Pro

Run the `/connect` command, select **ChatGPT (Codex OAuth)**, and choose the browser or headless login flow.

```txt
/connect
```

After login, OpenCode loads the available ChatGPT models in the background. The built-in model list remains available
when ChatGPT cannot be reached.
