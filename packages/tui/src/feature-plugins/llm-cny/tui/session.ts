...
export function hasChatGPTUsage(messages: ReadonlyArray<Message>) {
  return messages.some((item) => {
    if (item.role === "user") return item.model.providerID.toLowerCase() === "chatgpt"
    return item.providerID.toLowerCase() === "chatgpt"
  })
}
