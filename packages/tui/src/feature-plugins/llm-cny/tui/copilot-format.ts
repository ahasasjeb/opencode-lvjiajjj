...
export function formatSnapshot(snapshot: QuotaSnapshot, t?: Translator): string {
  if (snapshot.unlimited || snapshot.entitlement === -1 || (snapshot.entitlement === 0 && snapshot.unlimited)) {
    return t?.("plugin.llmCny.copilot.unlimited") ?? "Unlimited"
  }
...
export function formatResetDate(resetDate: string | null, locale = "en"): string {
  if (!resetDate) return ""
  try {
